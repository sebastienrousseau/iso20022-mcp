...
# iso20022-mcp Governance

This document describes how iso20022-mcp is run, how decisions are made,
...
iso20022-mcp is the routing gateway of the ISO 20022 MCP suite: one
small set of meta-tools (`search`, `list_families`, `list_servers`,
`describe`, `validate`, `generate`, `parse`) that route a message type to
the family server that owns it - [`pain001-mcp`](https://github.com/sebastienrousseau/pain001-mcp),
[`pacs008-mcp`](https://github.com/sebastienrousseau/pacs008-mcp),
[`camt053-mcp`](https://github.com/sebastienrousseau/camt053-mcp),
[`acmt001-mcp`](https://github.com/sebastienrousseau/acmt001-mcp) - so an
agent installs one thing and discovers the whole suite. The gateway does
no message work of its own. Changes are weighed against that scope:
...
- **Significant changes** (new public APIs, breaking changes, new
  dependencies, new families, new meta-tools/resources/prompts) need
  explicit approval from a maintainer in the PR, and should start as an
  issue or discussion. Decisions that shape the server are written down
...
Releases follow [`RELEASING.md`](RELEASING.md). iso20022-mcp carries its
own version number, incremented by 0.0.1 per release; the family servers
it routes to are optional extras with floors, not lockstep siblings. Only
maintainers publish to PyPI; release authority rests with the lead
maintainer and is expanding to a second maintainer as a standing goal.
...
1. Contribute a few reviewed PRs in an area
   ...
   catalogue entries in `registry.py`, framework adapters, examples).
...
iso20022-mcp today has **one** maintainer, which is a real risk for a
package used in banking. The mitigations in place:
